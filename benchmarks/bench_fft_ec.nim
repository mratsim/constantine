...
# ############################################################
#
#             Benchmark of FFT on Elliptic Curves
#
# ############################################################
...
proc bench_EC_FFT_NN_Recursive*() =
  echo "\n=== EC FFT (Natural → Natural, Recursive) Benchmark ==="
  separator()
...
proc bench_EC_FFT_NR_Iterative_DIF*() =
  echo "\n=== EC FFT (Natural → Bit-Reversed, Iterative DIF) Benchmark ==="
  separator()
...
proc bench_EC_FFT_RN_Iterative_DIT*() =
  echo "\n=== EC FFT (Bit-Reversed → Natural, Iterative DIT) Benchmark ==="
  separator()
...
proc bench_EC_FFT_NR_via_Recursive_and_BitRev*() =
  echo "\n=== EC FFT (Natural → Bit-Reversed, Recursive + BitRev) Benchmark ==="
  separator()
...
when isMainModule:
  echo "============================================================"
  echo "            FFT / IFFT Benchmarks - EC (BLS12-381 G1)"
  echo "============================================================"
  ...
  echo "  - EC FFT NN Rec:       Natural → Natural (recursive)"
  echo "  - EC FFT NR DIF:       Natural → Bit-Reversed (iterative DIF)"
  echo "  - EC FFT RN DIT:       Bit-Reversed → Natural (iterative DIT)"
  echo ""
  ...
  echo ""
  echo "--- Core Algorithms ---"
  bench_EC_FFT_NN_Recursive()
  bench_EC_FFT_NR_Iterative_DIF()
  bench_EC_FFT_RN_Iterative_DIT()
  echo ""
  echo "--- Combinations (Algorithm + Bit-Reversal) ---"
