...
proc bench_Fr_FFT*() =
  ...
  for scale in 1 ..< 10:
    ...
    bench("Fr FFT", "Fr[BLS12_381]", order, NumIters):
...
proc bench_Fr_IFFT*() =
  ...
  for scale in 1 ..< 10:
    ...
    bench("Fr IFFT", "Fr[BLS12_381]", order, NumIters):
...
proc bench_BitReversal*(T: typedesc) =
  ...
  for logN in countup(2, MaxLogN-1, 4):
    ...
    bench("Auto-dispatch (in-place)", name, N, NumIters):
      buf.bit_reversal_permutation()

    echo "----"

  echo "\n=== Bit-Reversal Permutation Benchmark (Out-of-Place) ==="
  separator()
  ...
  for logN in countup(2, MaxLogN-1, 4):
    ...
    bench("Auto-dispatch (oop)", name, N, NumIters):
