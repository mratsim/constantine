...
func sqrtAlg_NegDlogInSmallDyadicSubgroup_vartime(x: FF): int {.tags:[VarTime], raises: [].} =
  let key = cast[int](x.mres.limbs[0] and SecretWord 0xFFFF)
  return FF.Name.sqrtDlog(dlogLUT).getOrDefault(key, 0)
...
func sqrtAlg_GetPrecomputedRootOfUnity(target: var FF, multiplier: int, order: uint) =
  target = FF.Name.sqrtDlog(PrecomputedBlocks)[order][multiplier]

func invSqrtEqDyadic_vartime*(a: var FF) =
  ...
  var powers: array[4, FF]
  powers[0] = a
  for i in 1 ..< FF.Name.sqrtDlog(Blocks):
    powers[i] = powers[i - 1]
    for j in 0 ..< FF.Name.sqrtDlog(BlockSize):
  ...
  negExponent = sqrtAlg_NegDlogInSmallDyadicSubgroup_vartime(powers[FF.Name.sqrtDlog(Blocks) - 1])
  negExponent = negExponent shr FF.Name.sqrtDlog(FirstBlockUnusedBits)
  ...
  for i in 1 ..< FF.Name.sqrtDlog(Blocks):
    temp2 = powers[FF.Name.sqrtDlog(Blocks) - 1 - i]
    for j in 0 ..< i:
      sqrtAlg_GetPrecomputedRootOfUnity(temp, int( (negExponent shr (j*FF.Name.sqrtDlog(BlockSize))) and FF.Name.sqrtDlog(BitMask) ), uint(j + FF.Name.sqrtDlog(Blocks) - 1 - i))
      temp2.prod(temp2, temp)

    var newBits = sqrtAlg_NegDlogInSmallDyadicSubgroup_vartime(temp2)
    negExponent = negExponent or (newBits shl ((i*FF.Name.sqrtDlog(BlockSize)) - FF.Name.sqrtDlog(FirstBlockUnusedBits)))

  negExponent = negExponent shr 1
  a.setOne()

  for i in 0 ..< FF.Name.sqrtDlog(Blocks):
    sqrtAlg_GetPrecomputedRootOfUnity(temp, int((negExponent shr (i*FF.Name.sqrtDlog(BlockSize))) and FF.Name.sqrtDlog(BitMask)), uint(i))
    a.prod(a, temp)

func inv_sqrt_precomp_vartime*(r: var FF, a: FF) =
  var candidate, powLargestPowerOfTwo {.noInit.}: FF
  # Compute
