...
type
  ...
  AnyFFT_Descriptor*[F] = FrFFT_Descriptor[F] | CosetFFT_Descriptor[F]
    ## Concept for any FFT descriptor (standard or coset).
    ##
    ## Use this for function parameters when accepting both descriptor types.
    ## Both FrFFT_Descriptor and CosetFFT_Descriptor share the same fields:
    ##   - order: int
    ##   - rootsOfUnity: ptr UncheckedArray[F]
    ##
    ## Direct field access (desc.order, desc.rootsOfUnity) works in generic code,
    ## but sometimes not. (Generic sandwich bug?)
    ##
    ## The shift in CosetFFT_Descriptor is only applied by coset_fft_* functions,
    ## not by standard fft_* functions.
